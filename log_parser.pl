...
- Прибытие сообщения от адресата "<>" можно пропускать, т.к. это ошибка в формате заголовка From, который не удалось разобрать.
...
- Прибытие сообщения (запись с отправителем). Получить это сообщение можно по внутреннему ID, т.к. прибытие одно, а доставок может быть несколько.
	(как доп. информация для следующего списка).
